...
8. **QR on iPhone** uses the in-app camera (not the system sheet). Allow camera access; if scan still fails, rebuild after pulling latest — iOS requires barcode type `org.iso.QRCode`.
